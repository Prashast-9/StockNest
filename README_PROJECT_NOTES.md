...
├── sn_dashboard_view.html          → Analytics & Reporting page markup
├── sn_settings_view.html           → Settings page markup
├── assets/
│   ├── css/
│   │   └── sn_dashboard_style.css  → Shared styling for BOTH pages (layout, colors, responsive rules)
│   └── js/
│       ├── sn_dashboard_script.js  → Analytics page: chart rendering + button interactions
│       └── sn_settings_script.js   → Settings page: user table, audit log, SLA sliders, tabs
└── README_PROJECT_NOTES.md         → This file
```

Both HTML pages share the same sidebar/topbar and the same stylesheet, so the
look stays consistent as you navigate between them. Click **Analytics &
Reporting** or **Settings** in the sidebar to jump between the two pages.
