...
const googleLogin = async (req, res) => {
  ...
  if (!firebaseAdmin) {
    return res.status(503).json({
      message: 'Google Sign-in is not configured on this server (missing Firebase credentials).'
    });
  }
  ...
  try {
    ...
    // 2. Find user in local PostgreSQL database by email
    let userResult = await pool.query('SELECT * FROM users WHERE email = $1', [email]);
    let user = null;

    if (userResult.rows.length === 0) {
      // 3. User does not exist, create them
      // Find the first organization ID to assign them to
      const orgResult = await pool.query('SELECT org_id FROM organization ORDER BY org_id ASC LIMIT 1');
      if (orgResult.rows.length === 0) {
        return res.status(500).json({ message: 'No organizations configured in the database.' });
      }
      const orgId = orgResult.rows[0].org_id;

      // Create user with a randomized password hash since they use Google SSO
      ...
      const insertResult = await pool.query(
        `INSERT INTO users (name, email, password_hash, role, org_id)
         ...
         RETURNING user_id, name, email, role, org_id, created_at`,
        [name || 'Google User', email, passwordHash, orgId]
      );
      user = insertResult.rows[0];
      console.log(`🌱 Created new SSO user: ${email}`);
    } else {
